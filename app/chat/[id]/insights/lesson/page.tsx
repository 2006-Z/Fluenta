...
export default async function ChatInsightsLessonPage({
  params,
}: {
    ...
    <div className="flex min-h-[calc(100dvh-3rem)] flex-col">
      <div className="border-b border-border px-4 py-3">
        <div className="mx-auto flex w-full max-w-5xl items-center gap-3">
          ...
          <div>
            <h1 className="text-sm font-semibold text-foreground">Lesson</h1>
            <p className="text-xs text-muted">{insightsTitle(conversation)}</p>
          </div>
        </div>
      </div>
      <LessonPanel format={lesson.format} title={lesson.title} content={lesson.content} />
    </div>
  );
}
